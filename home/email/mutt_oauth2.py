...
from gi.repository import Secret  # ty:ignore[unresolved-import]
...
encrypted TOKENFILE.  Run with "--verbose --auth" to get started or whenever all
...
ap.add_argument("email", help="email address to authorise")
ap.add_argument(
    "-a", "--auth", action="store_true", help="manually authorise new tokens"
)
...
attributes = {"email": args.email, "app": "mutt_oauth2"}
token = Secret.password_lookup_sync(None, attributes)
if token is None:
    token = {}
else:
...
def writetokenfile():
    """Writes global token dictionary secret store"""
    Secret.password_store_sync(
        None,
        attributes,
        Secret.COLLECTION_DEFAULT,
        f"Mutt Oauth2 token for {args.email}",
        json.dumps(token),
    )
...
if not token:
    if not args.auth:
        sys.exit('You must run script with "--auth" at least once.')
...
baseparams: dict[str, Any] = {"client_id": registration["client_id"]}
...
if not access_token_valid():
    ...
    if not token["refresh_token"]:
        sys.exit('ERROR: No refresh token. Run script with "--auth".')
    p = baseparams.copy()
    p.update(
        {
            "client_secret": registration["client_secret"],
    ...
    if "error" in response:
        ...
        if "error_description" in response:
            print(response["error_description"])
        print('Perhaps refresh token invalid. Try running once with "--auth"')
        sys.exit(1)
    update_tokens(response)
